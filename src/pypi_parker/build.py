...
def _setup_body(setup_conf: SETUP_CONFIG) -> str:
    ...
    return os.linesep.join(
        ...
        config=",{linesep}    ".join(
            [
                f"{key}={value!r}"
                for key, value in sorted(setup_conf.items(), key=lambda item: item[0])
            ]
        ).format(linesep=os.linesep),
        allowed_suffixes=", ".join(
            repr(each) for each in sorted(ALLOWED_SETUP_SUFFIXES)
        ),
    )
...
def generate_and_build_package(
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        setup_py = SpecificTemporaryFile(
            name=os.path.join(tmpdirname, "setup.py"), body=_setup_body(package_config)
        )

        manifest_in = SpecificTemporaryFile(
            name=os.path.join(tmpdirname, "MANIFEST.in"), body=MANIFEST
        )
